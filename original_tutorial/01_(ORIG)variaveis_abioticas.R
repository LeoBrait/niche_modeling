...
# Download das vari?veis abioticas
...
# le os pacotes a serem utilizados
...
# readOGR()  fun??o para ler um shapefile
...
# rasters clim?ticos: utilizaremos os bioclimaticos do worldclim

setwd("G:/Meu Drive/Disciplinas ofertadas/2022 Disciplina Modelagem UESB/ENM/raster")
tif <- dir(pattern = "tif$")
tif
...
plot(var_br_05$bio_1)

# plot da variavel bio_1 (= annual Mean Temperature)

landscapetools::show_landscape(var_br$bio_1) +
  geom_polygon(data = var_br$bio_1 %>% raster::rasterToPolygons() %>% fortify, 
...
paste0("raster_br_res05_",colnames(var_da_cor06),".tif")%>%  
  file.copy(., "G:/Meu Drive/Disciplinas ofertadas/2022 Disciplina Modelagem UESB/ENM/03_var")
