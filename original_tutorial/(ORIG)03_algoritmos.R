...
# Prof. Luisa Maria Diele-Viegas & Juliana Hipolito
# -------------------------------------------------------------------------
...
# aqui a gente importa os rasters clim?ticos
...
for(i in occ$species %>% unique){ # para cada especie
  ...
  pa_species <- dismo::randomPoints(mask = var, n = nrow(pr_species)*10) %>% #*10 quando N de pontos for baixo
    tibble::as_tibble() %>%
    dplyr::rename(lon = x, lat = y) %>% 
    dplyr::mutate(id = seq(nrow(.)))
  
  
  pa_species_glm <- dismo::randomPoints(mask = var, n = 10000) %>% 
    tibble::as_tibble() %>%
    dplyr::rename(lon = x, lat = y) %>% 
    dplyr::mutate(id = seq(nrow(.)))
  ...
  for(r in replica %>% seq){	# numero de replicas do modelo 
    ...
    # dados de presenca
    pr_sample_train <- pr_species %>% 
      dplyr::sample_frac(partition) %>% 
      dplyr::select(id) %>% 
      dplyr::pull()
    
    # dados de pseudo ausencia
    pa_sample_train <- pa_species %>% 
      dplyr::sample_frac(partition) %>% 
      dplyr::select(id) %>% 
      dplyr::pull()
    
    # dados de pseudo ausencia
    pa_sample_train_glm <- pa_species_glm %>% 
      dplyr::sample_frac(partition) %>% 
      dplyr::select(id) %>% 
      dplyr::pull()
    gc()
